...
function LetterCard({ letter, onClick }: { letter: HebrewLetter; onClick: () => void }) {
  ...
  return (
    <motion.div
      whileTap={{ scale: 0.93 }}
      onClick={handleTap}
      className={`${letter.color} rounded-2xl p-3 shadow-md cursor-pointer select-none flex flex-col items-center gap-1 border border-white`}
    >
      <span dir="rtl" lang="he" className="text-5xl font-black text-blue-900 leading-none">{letter.letter}</span>
      <span className="text-xs font-black text-blue-700 uppercase tracking-wider">{letter.name}</span>
      <span className="text-xs text-blue-500 font-bold text-center leading-tight">{letter.phonetic === '—' ? 'silent' : `/${letter.phonetic}/`}</span>
      <AnimatePresence mode="wait">
        {flipped ? (
          <motion.div key="word" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <span dir="rtl" lang="he" className="text-sm font-black text-blue-800">{letter.word}</span>
            <span className="text-xs text-blue-500 font-bold">{letter.wordTranslit}</span>
            <span className="text-xs text-gray-500 font-bold">{letter.wordEnglish}</span>
          </motion.div>
        ) : (
...
function LettersMode() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-white/70 rounded-2xl p-3 shadow text-center">
        <p className="text-sm font-black text-blue-700">TAP A LETTER TO FLIP IT AND HEAR IT! 🔊</p>
        <p className="text-xs text-blue-400 font-bold mt-0.5">Hebrew has 22 letters — all read right to left →</p>
      </div>
        ...
        {categories.map(cat => (
          <motion.button key={cat} whileTap={{ scale: 0.92 }}
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-xl font-black text-xs shadow transition-all
              ${category === cat ? 'bg-blue-600 text-white' : 'bg-white/80 text-blue-600'}`}>
      ...
      <div className="grid grid-cols-3 gap-3">
        {filtered.map(w => (
          <motion.div
            ...
            className="bg-white rounded-2xl shadow-md p-3 cursor-pointer flex flex-col items-center gap-1 border border-blue-100"
          >
            <span className="text-3xl">{w.emoji}</span>
            <span dir="rtl" lang="he" className="text-xl font-black text-blue-900 leading-none">{w.word}</span>
            <AnimatePresence mode="wait">
              {flippedId === w.word ? (
                <motion.div key="detail" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="flex flex-col items-center gap-0.5 w-full">
                  <span className="text-xs font-black text-blue-500">{w.translit}</span>
                  <span className="text-xs font-bold text-gray-500 text-center leading-tight">{w.english}</span>
                </motion.div>
              ) : (
                <motion.span key="tap" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="text-xs text-blue-300 font-bold">tap!</motion.span>
              )}
            </AnimatePresence>
          </motion.div>
        ))}
...
function CompareMode() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-white/70 rounded-2xl p-3 shadow text-center">
        <p className="text-sm font-black text-blue-700">HEBREW vs ENGLISH — THE DIFFERENCES! 🤔</p>
      </div>
        ...
        {COMPARE_FACTS.map((f, i) => (
          <motion.button key={i} whileTap={{ scale: 0.9 }}
            ...
            className={`w-10 h-10 rounded-xl font-black text-lg shadow transition-all
              ${activeIdx === i ? 'bg-blue-600 text-white scale-110' : 'bg-white text-blue-400'}`}>
            {f.icon}
          </motion.button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeIdx}
          ...
          <div className="p-4 flex flex-col gap-3">
            {/* English side */}
            <div className="bg-sky-50 rounded-2xl p-3 border-2 border-sky-200">
              <p className="text-xs font-black text-sky-600 mb-1">🇬🇧 ENGLISH</p>
              <p className="text-sm font-bold text-sky-800">{fact.english}</p>
              <div className="mt-2 bg-white rounded-xl p-2 text-center">
                <span className="text-lg font-black text-sky-700 tracking-widest">{fact.demo.english}</span>
                {activeIdx === 0 && (
                  <div className="flex items-center justify-center gap-1 mt-1">
                    <span className="text-xs font-black text-sky-500">READ THIS WAY</span>
            ...
            <div className="bg-blue-50 rounded-2xl p-3 border-2 border-blue-200">
              <p className="text-xs font-black text-blue-600 mb-1">🇮🇱 HEBREW</p>
              <p className="text-sm font-bold text-blue-800">{fact.hebrew}</p>
              <div className="mt-2 bg-white rounded-xl p-2 text-center">
                {activeIdx === 0 ? (
                  <>
                    <div className="flex items-center justify-center gap-1 mb-1">
                      <motion.span animate={{ x: [0, -6, 0] }} transition={{ duration: 1, repeat: Infinity }}
                        className="text-lg">←</motion.span>
                      <span className="text-xs font-black text-blue-500">READ THIS WAY</span>
                    </div>
                    <span dir="rtl" lang="he" className="text-lg font-black text-blue-700 tracking-widest block">{fact.demo.hebrew}</span>
                  </>
                ) : (
                  <span dir="rtl" lang="he" className="text-lg font-black text-blue-700 tracking-widest">{fact.demo.hebrew}</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="flex gap-3 justify-center">
        <motion.button whileTap={{ scale: 0.92 }}
          ...
          className="px-5 py-2 rounded-xl font-black text-sm bg-white text-blue-600 shadow disabled:opacity-30">
          ← PREV
        </motion.button>
        <span className="flex items-center text-blue-400 font-black text-sm">
      ...
      <div className="bg-white rounded-2xl shadow p-4 text-center">
        <p className="text-xs font-black text-gray-500 mb-2">THE HEBREW ALPHABET IN ORDER (read right to left!)</p>
        ...
        {SUB_MODES.map(m => (
          <motion.button key={m.id} whileTap={{ scale: 0.92 }}
            ...
            className={`flex-1 py-3 rounded-xl font-black text-xs shadow flex flex-col items-center gap-1 transition-all
              ${subMode === m.id ? 'bg-blue-600 text-white scale-105 shadow-md' : 'bg-white/80 text-blue-700'}`}>
